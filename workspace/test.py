    ...
    while not FORCE_STOP:
        try:
            with lock:
                ...
                name, frame, result = event

                #event0 = audio_queue.get_nowait()
                #audio_queue.task_done()
                #name, frame, max_volume = event0
...
def Therad_recording(audio_queue:Queue, lock)->None:
    ...
    #if frames:
    #    max_volume = np.max(np.abs(frames))
    ...
    while not Force_stop:
        ...
        if name == "wav":
           ...
           #    spec_audio_queue.put(("spec_img", que_img))
            ...
            spec_audio_queue.put((name, frame, result_label))
